...
export function calcularActividad(actividadId, personas) {
  ...
  // Motos de agua: cada persona paga su suplemento normal (25€). La plaza
  // vacía de los grupos impares NO se reparte aquí — se cobra aparte como
  // suplemento de grupo (ver calcularSuplementoGrupo).
  return act.suplemento;
}

// Plazas que quedan vacías por tener que coger motos enteras (grupos impares).
export function plazasVaciasMotos(personas) {
  return Math.ceil(personas / 2) * 2 - personas;
}

/**
 * Coste que NO se puede repartir por persona y se suma al total del grupo.
 *
 * Caso motos de agua: se cogen en pareja. Si el grupo es impar sobra media
 * moto, y esa plaza vacía se paga ENTERA: no solo el suplemento de 25€, sino
 * también los ~20€ de valor de actividad que en las demás personas ya cubre
 * el pack que tienen contratado. Total 45€ por plaza vacía.
 *
 * Ejemplo: 7 personas con A Full + motos
 *   7 × (70 pack + 6 gestión + 25 motos) = 707€
 *   + 1 plaza vacía × 45€               =  45€
 *   TOTAL                                = 752€
 */
export function calcularSuplementoGrupo(actividadId, personas) {
  if (!actividadId) return 0;
  const act = catalog.actividades[actividadId];
  if (!act || act.calculo_especial !== 'motos_agua') return 0;

  const costePlazaVacia = act.coste_plaza_vacia || 45;
  return plazasVaciasMotos(personas) * costePlazaVacia;
}
...
export function calcularPresupuesto({
  ...
  // Costes que no se reparten por persona (plaza vacía de motos en grupos impares)
  const suplemento_grupo =
    calcularSuplementoGrupo(actividad, personas) +
    calcularSuplementoGrupo(segunda_actividad, personas);

  const total_grupo = por_persona * personas + suplemento_grupo;
  ...
  return {
    ...
    suplemento_grupo,
    plazas_vacias_motos:
      (actividad && catalog.actividades[actividad]?.calculo_especial === 'motos_agua') ||
      (segunda_actividad && catalog.actividades[segunda_actividad]?.calculo_especial === 'motos_agua')
        ? plazasVaciasMotos(personas)
        : 0,
    desglose,
    personas,
...
export function formatPresupuesto(p) {
  ...
  if (!p.solo_sabado) {
    mensaje += `*Viernes*\n`;
    // SUSPENDIDO TEMPORALMENTE (hasta nuevo aviso): entrada gratis Eclipse viernes.
  ...
  if (incluye.actividad) {
    // Si el cliente YA eligió actividad, mostrar el nombre concreto.
    // Si no, dejar el placeholder "Actividad a elegir".
    if (p.actividad_id) {
      const actNombre = catalog.actividades[p.actividad_id]?.nombre || p.actividad_id;
      mensaje += `🎯 ${actNombre}\n`;
    } else {
  ...
  if (incluye.comida) {
    if (p.comida_id) {
      const comNombre = catalog.comidas[p.comida_id]?.nombre || p.comida_id;
      mensaje += `🍽️ ${comNombre}\n`;
    } else {
  ...
  if (incluye.cena) {
    if (p.cena_id) {
      const cenNombre = catalog.cenas[p.cena_id]?.nombre || p.cena_id;
      mensaje += `🍷 ${cenNombre}\n`;
    } else {
  ...
  // Plaza de moto vacía (grupos impares): no se reparte, se suma al total.
  if (p.suplemento_grupo > 0) {
    const plazas = p.plazas_vacias_motos;
    mensaje += `🌊 + ${p.suplemento_grupo}€ por ${plazas === 1 ? 'la plaza de moto que queda libre' : 'las plazas de moto que quedan libres'} (sois impares)\n`;
