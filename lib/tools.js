...
import { updateConversation, getConversation, consultarDisponibilidadReserva } from './supabase.js';

// Capacidad de referencia por apartamento para calcular cuántos hacen falta
// cuando el chalet/villa están ocupados y caemos a apartamentos.
// 10 pers/apt funciona con los ejemplos de Diego (25 pers → 2-3 apartamentos).
const PERSONAS_POR_APARTAMENTO = 10;
...
export const TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        pack: {
          type: 'string',
          enum: ['basic', 'mix', 'afull', 'afull_sin_actividad', 'afull_sin_comida', 'premium', 'solo_actividad'],
          description: 'Pack elegido. Si el cliente no especifica, usa "afull" por defecto. Variantes de 50€: "afull_sin_actividad" si NO quieren actividad (mantienen comida+cena); "afull_sin_comida" si NO quieren comida (mantienen actividad+cena). "solo_actividad" (32€ base + suplemento actividad) si SOLO quieren una actividad, sin alojamiento ni comida/cena. NO mezclar.'
        },
        fecha: {
          type: 'string',
        ...
        alojamiento: {
          type: 'string',
          enum: ['hostal', 'bungalow', 'apartamento', 'hotel', 'chalet_kent', 'villa_bellreguard'],
          description: 'Tipo de alojamiento. chalet_kent (15 pax max, Gandía) y villa_bellreguard (20 pax max, Bellreguard) son dos opciones distintas con piscina, ambas valen lo mismo. Hasta 15 personas usa chalet_kent; 16+ personas usa villa_bellreguard. Omitir si no necesitan alojamiento.'
        },
        actividad: {
          type: 'string',
        ...
        solo_sabado: {
          type: 'boolean',
          description: 'TRUE si el cliente dijo explícitamente que viene solo el sábado (sin viernes). En ese caso el formato del presupuesto omite el bloque "Viernes" (Eclipse, chupitos, 2x1 pubs).'
        }
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        alojamiento: {
          type: 'string',
          enum: ['hostal', 'bungalow', 'apartamento', 'hotel', 'chalet_kent', 'villa_bellreguard', 'chalet', 'sin_alojamiento'],
  ...
  },
  {
    ...
    description: 'OBLIGATORIO antes de ofrecer Chalet Kent o Villa Bellreguard en un presupuesto. Comprueba si ese alojamiento está libre la fecha pedida. Si NO está disponible, devuelve el número de apartamentos sugeridos como alternativa — usa ese dato para ofrecer apartamentos directamente sin pasar por humano. Si SÍ está disponible, procede normalmente con calcular_presupuesto.',
    input_schema: {
      type: 'object',
      properties: {
        alojamiento: {
          type: 'string',
          enum: ['chalet_kent', 'villa_bellreguard'],
          description: 'Alojamiento a comprobar. Solo chalet_kent o villa_bellreguard tienen control de disponibilidad.'
        },
        fecha: {
          type: 'string',
          description: 'Fecha del sábado de la despedida en formato YYYY-MM-DD.'
        },
        personas: {
          type: 'number',
          description: 'Número de personas del grupo (necesario para calcular cuántos apartamentos alternativos).'
        }
      },
      required: ['alojamiento', 'fecha', 'personas']
    }
  },
  {
...
export async function executeTool(toolName, toolInput, context) {
  ...
  switch (toolName) {
    ...
    case 'consultar_disponibilidad': {
      try {
        const { alojamiento, fecha, personas } = toolInput;
        const r = await consultarDisponibilidadReserva(alojamiento, fecha);
        if (r.disponible) {
          return {
            ok: true,
            disponible: true,
            alojamiento,
            fecha,
            mensaje: `${alojamiento === 'chalet_kent' ? 'Chalet Kent' : 'Villa de Bellreguard'} disponible para ${fecha}. Procede con calcular_presupuesto.`
          };
        }
        // NO DISPONIBLE → calculamos cuántos apartamentos hacen falta
        const numApts = Math.max(1, Math.ceil(personas / PERSONAS_POR_APARTAMENTO));
        const nombre = alojamiento === 'chalet_kent' ? 'Chalet Kent' : 'Villa de Bellreguard';
        return {
          ok: true,
          disponible: false,
          alojamiento_solicitado: alojamiento,
          fecha,
          alternativa: {
            tipo: 'apartamento',
            num_apartamentos: numApts,
            personas
          },
          instruccion_al_bot: `${nombre} NO DISPONIBLE para ${fecha} (otra reserva). NO ofrezcas chalet ni villa para esta fecha. Sin avisar al humano, propon DIRECTAMENTE apartamentos (necesitaríais ${numApts} apartamento${numApts > 1 ? 's' : ''} para ${personas} personas). Llama YA a calcular_presupuesto con alojamiento='apartamento'. Dile al cliente algo natural tipo: "Para esa fecha el chalet y la villa ya están reservados, pero os monto el presupuesto con apartamentos que es lo más típico para grupos de ${personas} (necesitaríais ${numApts}). Échale un vistacito y me dices."`
        };
      } catch (err) {
        return { ok: false, error: err.message };
      }
