...
// --- Identificación robusta por ID de mensaje de Wassenger ---
// Cuando el bot envía un mensaje, Wassenger devuelve un ID. Lo guardamos
// y cuando llega message:out:new comparamos el msg.id contra esta lista.
// Más fiable que el hash del texto (que puede fallar por encoding, espacios, etc.).

export async function markBotSentMessageId(msgId) {
  if (!msgId) return;
  const key = `botsentid:${msgId}`;
  try {
    await redis('SET', key, '1', 'EX', '600'); // TTL 10 min
  } catch (e) {
    console.error('markBotSentMessageId failed:', e.message);
  }
}

export async function wasBotSentMessageId(msgId) {
  if (!msgId) return false;
  const key = `botsentid:${msgId}`;
  try {
    const exists = await redis('EXISTS', key);
    ...
    console.error('wasBotSentMessageId failed:', e.message);
    return false;
  }
}
