...
import { markBotSentMessage, markBotSentMedia, markBotSentMessageId } from './upstash.js';
...
export async function sendText(phone, message, deviceId) {
  ...
  // Capturar el ID que devuelve Wassenger para poder identificar el eco después
  const msgId = data.id || data.waId || data.messageId || data.data?.id;
  if (msgId) await markBotSentMessageId(msgId);
  return data;
}
...
export async function notifyHuman(reason, customerPhone, customerMsg, deviceId) {
  // Si el aviso es por un archivo recibido del cliente, añadimos la
  // instrucción del comando `confirmar` para que Diego pueda marcar la
  // reserva sin salir de su chat.
  const esArchivo = /\[ARCHIVO RECIBIDO DEL CLIENTE/i.test(customerMsg)
    || /archivo|imagen|documento|comprobante|formulario/i.test(reason);

  let message = `⚠️ *ONEPARTY BOT - Aviso*\n\n` +
  ...
  if (esArchivo) {
    message += `\n\n━━━━━━━━━━━━━━━━━━━━\n` +
      `💡 Si el archivo es el *recibo de la transferencia* y todo está OK, responde aquí:\n` +
      `\`confirmar ${customerPhone}\`\n\n` +
      `Eso bloquea la fecha del chalet/villa para que el bot no la ofrezca a nadie más.`;
  }
